...
# Verify that the logging function exists
if not hasattr(lib, "log_loop_indices"):
    raise RuntimeError("log_loop_indices not found in shared library!")

# Define function signatures for retrieving logged data
lib.get_logged_data.restype = ctypes.POINTER(LoopData)
lib.get_logged_data.argtypes = [ctypes.POINTER(ctypes.c_size_t)]
lib.clear_log.argtypes = []

def retrieve_logged_data(output_file: str = "/home/jjlab/dump/logged_data.json"):
    """Retrieve and save logged loop data to a JSON file."""
    size = ctypes.c_size_t()
    data_ptr = lib.get_logged_data(ctypes.byref(size))

    if data_ptr:
        print(f"Retrieved {size.value} elements.")
        # Collect the data in a structured format
        log_data = []
        for i in range(size.value):
            entry = data_ptr[i]
            func_name = entry.func_name.decode('utf-8').rstrip('\x00')
            log_entry = {
                "function": func_name,
                "i": entry.i,
                "j": entry.j,
        ...
        print(f"Logged data saved to {output_file}")
        lib.clear_log()  # Clear the log after processing
    else:
...
class DebugChat:  # pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        # Retrieve all functions available in the module
        print("Attributes of self.mod:")
        print(type(self.mod))
    ...
    #     """Retrieve the logged loop data from the shared library."""
    #     size = ctypes.c_size_t()
    #     data_ptr = lib.get_logged_data(ctypes.byref(size))
    ...
    #         for i in range(size.value):
    #             entry = data_ptr[i]
    #             func_name = entry.func_name.decode('utf-8').rstrip('\x00')  # Clean name
    #             print(
    #                 f"Function: {func_name}, i: {entry.i}, j: {entry.j}, k: {entry.k}, "
    #                 f"tile_i: {entry.tile_i}, tile_j: {entry.tile_j}, tile_k: {entry.tile_k}"
    #             )
    #         lib.clear_log()  # Clear the log after reading
    #     else:
    #         print("No logs available.")
    ...
    def generate(
        ...
        print("======================= Loop Logs =======================")
        retrieve_logged_data()  # Retrieve the logged loop indices
        print("======================= Starts Decode =======================")
        for i in range(generate_length - 1):
            ...
            if next_token in self.conversation.stop_token_ids:
                break
        #print("======================= Loop Logs Decode=======================")
        #self.retrieve_log()  # Retrieve the logged loop indices
